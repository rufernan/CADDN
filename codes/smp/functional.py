...
def compute_metrics(output, target):
    ...
    # in case of a one-hot-encoding, we assume the positive class in the first band
    if output.shape[1]>1:
        output = output[:,0:1,:,:]
        target = target[:,0:1,:,:]
    ...
    # Computing metrics with required reduction 
    iou = iou_score(tp, fp, fn, tn, reduction="micro-imagewise")
    f1 = f1_score(tp, fp, fn, tn, reduction="micro-imagewise")
    f2 = fbeta_score(tp, fp, fn, tn, beta=2, reduction="micro-imagewise")
    acc = accuracy(tp, fp, fn, tn, reduction="micro-imagewise")
    rec = recall(tp, fp, fn, tn, reduction="micro-imagewise")
    
    # Error rates per class (computing the two internal terms of BER)
    fnr = false_negative_rate(tp, fp, fn, tn, reduction="micro-imagewise") # FN/(FN+TP) <-- error rate positive class
    fpr = false_positive_rate(tp, fp, fn, tn, reduction="micro-imagewise") # FP/(FP+TN) <-- error rate positive class
    ber = (fpr+fnr)/2.0
    
    return (iou, f1, f2, acc, rec, ber, fnr, fpr)
